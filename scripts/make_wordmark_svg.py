...
All modes open with the same left-to-right wipe, then differ in the rotation:
  rock   -- oscillates +/-11 deg around the rest pose, forever (this is the one
   ...
   shuffle -- GSAP-style scramble: random glyphs resolve into the word
              left-to-right, hold, then re-scramble and loop forever
...
def main():
    ...
    ap.add_argument("--mode", choices=["spin", "once", "rock", "shuffle", "static"], default="rock")
    ...
    if a.mode == "spin":
        ...
        import random
        rnd = random.Random(1337)             # deterministic scramble
        nf = a.frames or 26
        dur = a.dur or 6.0
        hold = 12                             # extra resolved copies -> long hold
        proj = [project(P, N, rest)]
        scale, cx, cy = fit(proj)
        base = rasterize(proj[0], scale, cx, cy)
        col_frac = COLS / nf
        ...
        for i in range(nf):
            limit = col_frac * (i + 1)
            frames.append([
                "".join(ch if ch != " " and x <= limit else
                        (" " if ch == " " else rnd.choice(RAMP[1:]))
